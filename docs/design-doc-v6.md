...
- **Backlog, not scheduled to a phase**: switch access/refresh/room tokens from plain signed JWTs to JWE (encrypted JWT). Discussed post-Phase-3 (2026-09-02) after noticing every token starts with the same base64 string — that's just the (intentionally public) `{"alg":"HS256","typ":"JWT"}` header, not a weakness; HS256's signature already prevents forgery without the secret. JWE would hide payload claims (`sub`/`did`/`room`/etc.) from anyone holding the token, which isn't a threat this app currently has (the holder is the same person the claims describe) — worth a real threat-model discussion before implementing, not an assumed upgrade.
...
**Validation strategy**: `scripts/network-emulation.sh` (host-level tc netem + iptables, keyed by one peer's LAN IP — asymmetric delay/jitter/loss, and simulated DPI-style UDP blocking that forces the TURN/TLS fallback) and `scripts/toxiproxy-scenarios.sh` (scripted, timed cuts on the TURN/TLS-443 path specifically, for on-demand ICE-restart testing) against two instances of `testing/webrtc-harness/` on the local docker-compose stack. Doesn't depend on the counterparty's location or a finished client — real-link validation over the actual India/Russia link happens later, at Phase 8 (§13).
...
**Status: done, validated 2026-09-02.** Two `testing/webrtc-harness/` tabs held a real two-participant call against the local docker-compose stack (simulcast confirmed active via LiveKit's own `mediaTrack published` logs showing 3 VP8 layers per participant). The local Docker Desktop/WSL2 environment turned out to be intermittently unstable on its own for sustained UDP media — every ~15-20s, both peers hit a `reconnecting → connected → reconnected → resubscribed` cycle, recovering within 1-3 seconds every time, dozens of times over the session, with zero full call drops on either side. That's the done-bar's actual requirement (ICE restart recovering from a real hiccup without a full drop) being met organically and repeatedly, which is stronger evidence than a single scripted cut would have given.

The scripted `toxiproxy-scenarios.sh cut` was run for completeness but had no observable effect, because every connection in this session negotiated a direct `host` UDP candidate — toxiproxy only sits in front of coturn's TURN/TLS path, so cutting it does nothing unless a call is actually routing through TURN. Forcing relay selection via `network-emulation.sh --block-udp` wasn't cleanly exercisable on this particular Windows/Docker-Desktop-WSL2 setup (`tc`/`iptables` need a real Linux netns, which Git Bash/MSYS2 doesn't provide, and WSL2's own virtualized network path adds a layer the script wasn't written against) — noted here rather than silently skipped. The scripts themselves work correctly (`setup`/`cut`/`restore`/`teardown` all behaved as expected against the proxy) and remain available for a Linux host or a future CI runner where this is straightforward.
...
Capacity check under real load, redeploy rehearsal, final sign-off on the §9 accepted-risk table and the §10.2a foreground-notification decision, real India/Russia link validation for call quality (moved from Phase 3 — see §13 Phase 3's note), and — if it hasn't already forced itself in earlier — the production hosting decision deferred in §1a.
**Done when**: the 5-participant call + messaging + local-machine-load test in §8 runs clean, a week of real calls over the actual India/Russia link shows ICE-restart recovering from at least one real network hiccup without a full call drop with the quality dashboard/logs showing which candidate type (host/srflx/relay-UDP/relay-TCP) won each call, the Phase 7 rehearsal has a real "under an hour" number attached to it, and both open decisions in §12 have been made (not just noted) by the two of you.
